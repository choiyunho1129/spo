...
greso_p_hard=${GRESO_P_HARD:-0.5}
greso_target_zero_variance=${GRESO_TARGET_ZERO_VARIANCE:-0.25}
greso_default_br_size=${GRESO_DEFAULT_BR_SIZE:-192}
greso_min_p=${GRESO_MIN_P:-0.05}
greso_max_p=${GRESO_MAX_P:-0.95}
greso_beta=${GRESO_BETA:-1.25}

python3 -m recipe.greso.main_greso \
    ...
    data.p_easy=${greso_p_easy} \
    data.p_hard=${greso_p_hard} \
    data.target_zero_variance=${greso_target_zero_variance} \
    data.default_br_size=${greso_default_br_size} \
    data.min_p=${greso_min_p} \
    data.max_p=${greso_max_p} \
    data.beta=${greso_beta}
